...
@Component({
  selector: 'app-bettor-report',
  templateUrl: './bettor-report.component.html',
  styleUrls: ['./bettor-report.component.scss']
})
export class BettorReportComponent implements OnInit {

  public p:any;

  public sport:number = 0;
  public result_type:string='operator';
  public bettors:any[] = [];
  ...
  on_search()
  {
    ...
    this.adminService.get_report_of_bettors(this.cachService.getToken(), filter).then(res =>{
      if(res)
      {
        svc.result_type = res.result_type;
        svc.bettors = res.bettors;
        ...
        this.bettors.forEach(bettor =>{
          this.result_bettors.push({name: bettor.name, value: Number(bettor.bets)})
        })
      }      
    });    
  }
